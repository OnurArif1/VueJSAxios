<template>
  <div class="container pt-5">
    ...
    <form @submit.prevent="onSubmit">
      ...
      <button @click="$router.push('/')" class="btn btn-danger">İptal</button> <!--burada router.push sayesinde iptal butonuna bastığında '/' bu dizine git dedik-->
...
import axios from 'axios'
  export default {
    ...
    methods : {
      onSubmit(){
        axios.post("/posts.json",{ ...this.post, updatedDate : new Date()}) //post 'un içindeki bütün propetyleri alıyor ve güncelliyor
          .then(response =>{
            console.log(response)
            this.post={} //kaydete bastığında yazılan yerleri boşalt dedik
          })
